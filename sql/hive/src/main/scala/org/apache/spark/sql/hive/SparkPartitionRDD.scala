...
import org.apache.spark.mapred.{CombineFileRecordReader, CombineSparkInputFormat, CurrentPath}
...
class SparkPartitionRDD[K, V](
  ...
  override def getPartitions: Array[Partition] = {
    ...
    val combine = new CombineSparkInputFormat[Writable, Writable]()
    combine.setMaxSplitSize(sqlc.conf.mapperSplitSize)
  ...
  override def compute(theSplit: Partition, context: TaskContext): InterruptibleIterator[(K, V)] = {
    val iter = new NextIterator[(K, V)] {
      ...
      var reader = new CombineFileRecordReader(
        jobConf, split.inputSplit.value.asInstanceOf[CombineFileSplit], Reporter.NULL, inputFormat)
